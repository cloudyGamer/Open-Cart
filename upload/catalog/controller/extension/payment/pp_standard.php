...
class ControllerExtensionPaymentPPStandard extends Controller {
	public function index() {
        ...
        // Create the logger
        $logger = new Logger('pp_standard');
        // Now add some handlers
        $logger->pushHandler(new StreamHandler('/home/customer/www/pauldowlingportfolio.com/{file path}', Logger::DEBUG));
        $logger->pushHandler(new FirePHPHandler());
		// You can now use your logger
        $logger->info('pp_standard index called'.__Line__);
		//addition - test if I can load the view from my front-end - I recieve it as a response
		/////////
		...
		if (!$this->config->get('payment_pp_standard_test')) {
			$data['action'] = 'https://www.paypal.com/cgi-bin/webscr&pal={key}';
			//echo "youve been redirected";
		} else {
			$data['action'] = 'https://www.sandbox.paypal.com/cgi-bin/webscr&pal={key}'; 
		...
		if ($order_info) {
			...
			$data['invoice'] = $this->session->data['order_status_id']. ' - ' . $order_info['payment_firstname'] . ' ' . $order_info['payment_lastname'];
			...
			if (!$this->config->get('payment_pp_standard_transaction')) {
				...
				//this is the condition the code follows
		...
		$logger->info('data not sent to order');

		
	}

	public function apiPayPal() {	
		...
		// You can now use your logger
        $logger->info('pp_standard index called');
		//addition - test if I can load the view from my front-end - I recieve it as a response
        //return $this->load->view('extension/payment/pp_standard');
		/////////
		$this->load->language('extension/payment/pp_standard');
        ...
        $url = 'https://www.sandbox.paypal.com/cgi-bin/webscr&pal={key}'; 
        $cURLData = $data['products'];
        echo "cURLData is equal to....".var_dump($cURLData);
        unset($data['products']);
        echo "cURLData is now equal to....".var_dump($cURLData);
        ...
        $response = do_curl_request($url,$data,$cURLData);
        //original//      
	...
	public function callback() {
        ...
        $logger->info('pp_standard callback called');
        ...
        //interesting 3 - here is where the data is sent via cURL
		$this->load->model('checkout/order');
