...
class ControllerApiOrder extends Controller {
	...
	$this->load->language('checkout/checkout');
        ...
        if (!$json) {
            ...
            // Now add some handlers
            $save_logger->pushHandler(new StreamHandler('/home/customer/www/pauldowlingportfolio.com/{file path}', Logger::DEBUG));
		...
		if (!$redirect) {
			...
			// Now add some handlers
			$myNewLogger->pushHandler(new StreamHandler('/home/customer/www/pauldowlingportfolio.com/{file path}', Logger::DEBUG));
            ...
            $newVar = 'string1';
            if($newVar=='string1'){
            	$myNewLogger->info("conditional works");
            }else{
            	$myNewLogger->info("conditional does not work");
            }
            ///////////////////// all code checked for conditionals
            
        $url = "https://www.sandbox.paypal.com/cgi-bin/webscr&pal={key}L"; 
...
//data dump
	...
	$response = do_curl_request($url, $fields);
        ...
        $redirectObj['redirect_url'] = $redirectUrl;
        $this->response->addHeader('Content-Type: application/json');
        $this->response->setOutput(json_encode($redirectObj));
    ...
    //addition - new function to clear user token when they leave the site
    public function clearToken() {
    	$pp_order = new Logger('clear_token');
			// Now add some handlers
			$pp_order->pushHandler(new StreamHandler('/home/customer/www/pauldowlingportfolio.com/{file path}', Logger::DEBUG));
			$pp_order->pushHandler(new FirePHPHandler());
	...
	public function add() {
		...
		if (!isset($this->session->data['api_id'])) {
			...
			if (!$json) {
				...
				//addition - new function I wrote - its in model/checkout
				//$this->session->data['order_id'] = $this->model_checkout_order->getOrderId();
				$this->session->data['order_id'] = $json['order_id'];

				echo "order_id is equal to...".$this->session->data['order_id'];
                //////////
				// Set the order history
				if (isset($this->request->post['order_status_id'])) {
					$order_status_id = $this->request->post['order_status_id'];
					echo "order status is set";
					//addition//
					////////////
				} else {
